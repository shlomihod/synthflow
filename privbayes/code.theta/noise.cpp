...
double noise::nextGaussian(engine& eng, double std) {
	boost::random::normal_distribution<double> normal(0.0, std);
	return normal(eng);
}
...
double noise::nextDouble(engine& eng, double range) {
	boost::random::uniform_real_distribution<double> unif(0.0, range);
	return unif(eng);
}

double noise::nextDouble(engine& eng, double left, double right) {
	boost::random::uniform_real_distribution<double> unif(left, right);
	return unif(eng);
}

int noise::nextInt(engine& eng, int range) {			// right-exclusive
	boost::random::uniform_int_distribution<int> unif(0, range - 1);
	return unif(eng);
}

int noise::nextInt(engine& eng, int left, int right) {	// right-exclusive
	boost::random::uniform_int_distribution<int> unif(left, right - 1);
	return unif(eng);
}
...
double noise::nextExponential(engine& eng, double lambda) {
	boost::random::exponential_distribution<double> expo(lambda);
	return expo(eng);
}
...
int noise::nextGeometric(engine& eng, double p) {
	boost::random::geometric_distribution<int> geometric(p);
...
double noise::nextCauchy(engine& eng, double scale) {
	boost::random::cauchy_distribution<double> cauchy(0.0, scale);
	return cauchy(eng);
}
