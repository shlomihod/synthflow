...
// https://www.cs.colostate.edu/~cs253/Fall21/Lecture/Manipulators
void show_double_hex(double x) {
...
////////////////////////////// bayesian //////////////////////////////
bayesian::bayesian(engine& eng1, table& tbl1, double ep, double split, double theta) : base(eng1, tbl1) {
	dim = tbl.dim;
...
vector<dependence> bayesian::greedy(double ep) {
	...
	for (int t = 0; t < dim; t++) {
		...
		cout << "EM[" << t << "] deps.size = " << deps.size() << endl;
		// cout << deps.size() << "\t";

		cout << "\tEM quality scores" << endl;
		vector<double> quality;
		for (const auto& dep : deps) {
			score = tbl.getScore(dep); 
			quality.push_back(score);
			cout << "\t\t" << to_string(dep) << " "; 
			show_double_hex(score);
			cout << endl;	
		}
		...
		cout << "\tEM picked = " << to_string(picked) << endl;
	}
	return model;
}
...
void bayesian::addnoise(double ep) {
	...
	for (const dependence& dep : model) {
		cout << "Dep " << to_string(dep) << " with Laplace noise scale " << 2.0 * dim / ep << endl;
		vector<double>& counts_syn = syn.margins[dep.cols].counts[dep.lvls];
		for (double count : tbl.getCounts(dep.cols, dep.lvls)) {
			random_noise = noise::nextLaplace(eng, 2.0 * dim / ep);
			noisy_count = count + random_noise; 
			counts_syn.push_back(noisy_count);
			cout << "\t" << count << " ";
			show_double_hex(random_noise);
			cout << " ";
			show_double_hex(noisy_count);
			cout << endl;
		}
	}
	// add consistency
