...
bayesian::bayesian(engine& eng1, table& tbl1, double ep, double split, int degree) : base(eng1, tbl1) {
	dim = tbl.dim;
	marginals_size = degree + 1;
	// SH: bound = 2^k
	...
	*/
	...
	cout << "Greedy call with epsilon: " << split * ep << endl;
	model = greedy(split * ep);
	cout << "Add noise call with epsilon: " << (1-split) * ep << endl;
	addnoise((1-split) * ep);
...
vector<dependence> bayesian::greedy(double ep) {
	// SH: `dependence` is a vector of (attr, attrs) pairs
	vector<dependence> model;
	double sens = tbl.sens;

	// SH: attributes that are already introduced to the network (so can be conditioned)
	set<int> S;
	// SH: attributes that are not yet introduced to the network
	...
	for (int t = 0; t < dim; t++) {
		...
		// SH: calculate maximal mutual information
...
vector<dependence> bayesian::S2V(const set<int>& S, const set<int>& V) {
	...
	for (int x : V) {
		// SH: set of possible parents
		set<vector<attribute>> exist;
		// SH: we get vector of possible parents
		vector<vector<attribute>> parents = maximal(S, marginals_size);
...
// SH: create all possible dependencies (num_vertices over marginals_size)
vector<vector<attribute>> bayesian::maximal(set<int> S, int marginals_size) {
	vector<vector<attribute>> ans;
	if (marginals_size == 0) return ans;
	if (S.empty()) {
	...
	for (int l = 0; l < depth; l++) {
		...
		vector<vector<attribute>> maxs = maximal(S, marginals_size - 1);
	...
	// without 'last'
	vector<vector<attribute>> maxs = maximal(S, marginals_size);
	for (vector<attribute> z : maxs)
		if (exist.find(z) == exist.end()) {
			exist.insert(z);
			ans.push_back(z);
